...
class ForumCommandCog(commands.Cog):
    ...
    async def forum_account_command(self, ctx: Context, username: str):
    ...
    async def on_forum_account_command_error(self, ctx: Context, error):
    ...
    async def forum_gift_command(self, ctx: Context):
        pass
    ...
    async def diamonds_subcommand(self, ctx: Context, user: str, amount: int):
        res = await self.client.api.get(Route(
            "/private/game/gift",
    ...
    @commands.bot_has_guild_permissions(send_messages=True)
    @command_checks.is_theotown_developer()
    async def region_coins_subcommand(self, ctx: Context, user: str, amount: int):
        res = await self.client.api.get(Route(
            "/private/game/gift",
    ...
    @commands.bot_has_permissions(send_messages=True)
    @command_checks.is_theotown_developer()
    async def forum_activate_command(self, ctx: Context, user: str):
        async with ctx.typing():
            res = await self.client.api.get(Route(
    ...
    async def on_forum_activate_command_error(self, ctx: Context, error):
        if isinstance(error, MissingRequiredArgument):
            if error.param.name == "username":
